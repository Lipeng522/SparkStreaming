 ...
 *内存优化：
 *     （1）RDD + JAVA序列化 1731
 *     （2）将java序列化更换成kryo序列化，设置序列化缓存级别（RDD+kryo）
 *     .set("spark.serializer","org.apache.spark.serializer.KryoSerializer")
 *     .registerKryoClasses(Array(classOf[DwsMember]))
 *     开启序列化缓存级别
 *     result.persist(StorageLevel.MEMORY_AND_DISK_SER)
 *
 *     结果：storgage中内存优化约10倍，原2g 优化后200mb左右
 *
 *     （3）dataset的默认序列化（优化）
 *     使用dataset优化后，storage内存占用约30MB左右
 *     更换缓存级别之后
 *
 *     （4）将dataset的缓存级别更换成序列化内存级别，默认是内存级别，优化后30MB左右
 *
 * 分区优化：
 *     （1）分区数量一般为cpu核数的为2:1或者3:1,最大使用cpu效率，防止cpu空转
 *
 * shuffle参数调整，适合大数据场景的优化
...
object DwsMemberService {
  ...
  def importMemberUseApi(sparkSession: SparkSession, dt: String) = {
    ...
    //===的方式必须两个表中的字段名不相同；相同只能用join内使用seq的方式
