...
object EtlDataService{
  ...
  def etlBaseAdLog(ssc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._ //隐式转换，用于使用toDF和toDS
    ssc.textFile("/user/atguigu/ods/baseadlog.log").filter(item => {
      val obj = ParseJsonData.getJsonData(item)
      obj.isInstanceOf[JSONObject]
    }).mapPartitions(partition => {  //mapPartition可以并行执行，map的话只有一线程处理，效率低
      partition.map(item => {
        val jsonObject = ParseJsonData.getJsonData(item)
        ...
        val dn = jsonObject.getString("dn")
        (adid, adname, dn)
      })  //减少分区数，是为了减少写出文件的数量，避免小文件，不shuffle直接减少分区，可能会导致oom，若想避免oom则可以开启shuffle减少分区
    }).toDF().coalesce(1).write.mode(SaveMode.Overwrite).insertInto("dwd.dwd_base_ad")
      //insertinto是支持hive的，必须提前创建表，插入的字段顺序与表中一致
      //saveastable不支持hive，会自动创建表，但插入的字段名必须与表中字段名相同
  }
